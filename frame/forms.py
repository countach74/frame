...
class BasicForm(object):
	...
	def render(self, action, fields=None, buttons=[SubmitType()], method='post', **kwargs):
		elements = []
		...
		for key, value in self.structure.items():
			if (fields and key in fields) or not fields:
				if isinstance(value, CustomType):
					...
					elements.append(value.make_form_element(key, data_item))
	
				else:
					elements.append(CustomType.make_form_element(key, data_item))
